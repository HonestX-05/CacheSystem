...
void testHotDataAccess() {
    ...
    CachePolicys::LruCache<int, std::string> lru(CAPACITY);
    CachePolicys::LfuCache<int, std::string> lfu(CAPACITY);
    CachePolicys::ArcCache<int, std::string> arc(CAPACITY);
    // 为LRU-K设置合适的参数：
    // - 主缓存容量与其他算法相同
    // - 历史记录容量设为可能访问的所有键数量
    // - k=2表示数据被访问2次后才会进入缓存，适合区分热点和冷数据
    CachePolicys::LruKCache<int, std::string> lruk(CAPACITY, HOT_KEYS + COLD_KEYS, 2);
    CachePolicys::LfuCache<int, std::string> lfuAging(CAPACITY, 20000);

    std::random_device rd;
    std::mt19937 gen(rd());
    
    // 基类指针指向派生类对象，添加LFU-Aging
    std::array<CachePolicys::CachePolicy<int, std::string>*, 5> caches = {&lru, &lfu, &arc, &lruk, &lfuAging};
...
void testLoopPattern() {
    ...
    CachePolicys::LruKCache<int, std::string> lruk(CAPACITY, LOOP_SIZE * 2, 2);
    CachePolicys::LfuCache<int, std::string> lfuAging(CAPACITY, 3000);
...
void testWorkloadShift() {
    ...
    CachePolicys::LfuCache<int, std::string> lfu(CAPACITY);
    CachePolicys::ArcCache<int, std::string> arc(CAPACITY);
    CachePolicys::LruKCache<int, std::string> lruk(CAPACITY, 500, 2);
    CachePolicys::LfuCache<int, std::string> lfuAging(CAPACITY, 10000);
